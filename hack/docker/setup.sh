...
DIST=$GOPATH/src/github.com/displague/csi-linode/dist
mkdir -p $DIST
if [ -f "$DIST/.tag" ]; then
...
clean() {
  pushd $GOPATH/src/github.com/displague/csi-linode/hack/docker
  rm csi-linode Dockerfile
  popd
}

build_binary() {
  pushd $GOPATH/src/github.com/displague/csi-linode
  ./hack/builddeps.sh
  ./hack/make.py build
  detect_tag $DIST/.tag
  popd
