...
// falls back to staticToken (e.g. from --LINODE_TOKEN / envflag), then to the
// LINODE_TOKEN environment variable. Returns an error when no source yields a token.
...
func TokenProviderFromFileOrEnv(ctx context.Context, staticToken string) (TokenProvider, string, error) {
	...
	if token := strings.TrimSpace(staticToken); token != "" {
		return StaticTokenProvider(token), fmt.Sprintf("flag or environment variable %q", AccessTokenEnv), nil
	}
